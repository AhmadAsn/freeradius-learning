...
```mermaid
graph TD
    VIP["Keepalived VIP<br/>10.0.0.100"] --> A["Host A (MASTER)<br/>RADIUS + MariaDB (primary)"]
    VIP --> B["Host B (BACKUP)<br/>RADIUS + MariaDB (replica)"]
    A -->|"Replication"| B

    style VIP fill:#495057,color:#fff
...
```mermaid
graph TD
    A["Host A<br/>RADIUS"] --- G[("MariaDB Galera Cluster")]
    B["Host B<br/>RADIUS"] --- G

    NAS["🖧 NAS Devices"] -->|"Primary: 10.0.0.1"| A
    NAS -->|"Secondary: 10.0.0.2"| B
...
```mermaid
graph TD
    LB["HAProxy / F5<br/>UDP Load Balancer<br/>:1812 / :1813"] --> A["RADIUS Host A"]
    LB --> B["RADIUS Host B"]
